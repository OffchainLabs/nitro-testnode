...
# conditionals (if/||/&&) or the error will be silently swallowed.
  ...
  if [ ${#BACKGROUND_PIDS[@]} -gt 0 ]; then
    ...
    for pid in "${BACKGROUND_PIDS[@]}"; do
      if ! wait "$pid" 2>/dev/null; then
        echo "Note: background process $pid exited with non-zero status" >&2
      fi
    done
  fi
  # Force-stop and remove any remaining scripts containers (orphaned or still running)
  docker compose rm -sf scripts 2>/dev/null || true
}
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --l3-fee-token-decimals)
            ...
            if ! [[ "$l3_custom_fee_token_decimals" =~ ^[0-9]+$ ]] || [[ $l3_custom_fee_token_decimals -lt 0 || $l3_custom_fee_token_decimals -gt 36 ]]; then
...
if $force_init; then
    ...
    docker volume prune -f --filter "$COMPOSE_LABEL_FILTER" || echo "Warning: volume prune failed" >&2
    ...
    if $l2referenceda; then
        ...
        l2referenceDAOutput=$(docker compose run --rm -e DEPLOYER_PRIVKEY="$l2ownerKey" --entrypoint bash rollupcreator -c "set -eo pipefail; cd /contracts-local && forge create src/osp/ReferenceDAProofValidator.sol:ReferenceDAProofValidator --rpc-url http://geth:8545 --private-key \$DEPLOYER_PRIVKEY --broadcast --constructor-args [$referenceDASignerAddress]") || {
            echo "Error: forge create failed for ReferenceDAProofValidator" >&2
            exit 1
        }
        l2referenceDAValidatorAddress=$(echo "$l2referenceDAOutput" | awk '/Deployed to:/ {print $NF}')
        if [ -z "$l2referenceDAValidatorAddress" ]; then
            echo "Error: failed to extract ReferenceDAProofValidator address from forge output:" >&2
            echo "$l2referenceDAOutput" >&2
            exit 1
        fi
