...
function writeConfigs(argv: any) {
    ...
    posterConfig["http"]["port"] = 8147
    posterConfig["ws"]["port"] = 8148
...
function writeEvilConfig(argv: any) {
    ...
    let validatorConfig = JSON.parse(baseConfJSON)
    validatorConfig["parent-chain"].wallet.account = namedAccount("evil-validator").address
    validatorConfig.node.staker.enable = false // DISABLE STAKER, use BOLD instead.
    validatorConfig.node.bold.enable = true;
    validatorConfig.node.staker["use-smart-contract-wallet"] = true
    let validconfJSON = JSON.stringify(validatorConfig)
    ...
    fs.writeFileSync(path.join(consts.configpath, "evil_validation_node_config.json"), JSON.stringify(validationNodeConfig))
}
...
export const writeEvilConfigCommand = {
    command: "write-evil-config",
    describe: "writes evil config files",
    handler: (argv: any) => {
        writeEvilConfig(argv)
    }
}
