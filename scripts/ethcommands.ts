...
// ethers.js error codes that indicate logic/argument errors rather than connectivity issues.
...
// Poll conditionFn every intervalMs milliseconds. Throws with errorMsg if
// timeoutMs elapses without the condition returning true. Transient errors
// from conditionFn are logged and retried; programming errors (TypeError,
// ReferenceError, RangeError) and known non-transient ethers.js errors
// (see NON_TRANSIENT_ERROR_CODES) abort immediately.
async function pollUntil(conditionFn: () => Promise<boolean>, intervalMs: number, timeoutMs: number, errorMsg: string): Promise<void> {
    if (intervalMs <= 0) throw new Error(`pollUntil: intervalMs must be positive, got ${intervalMs}`);
    if (timeoutMs <= 0) throw new Error(`pollUntil: timeoutMs must be positive, got ${timeoutMs}`);
    ...
    while (true) {
        ...
        try {
            ...
            if (e instanceof TypeError || e instanceof ReferenceError || e instanceof RangeError) {
                throw new Error(`${errorMsg}: programming error: ${e.message}`);
            }
            // Abort immediately on errors that are clearly not transient
            if (NON_TRANSIENT_ERROR_CODES.has(e.code)) {
                throw new Error(`${errorMsg}: non-transient error: ${e.message}`);
            }
...
async function sendTransaction(argv: any, threadId: number) {
    ...
    for (let index = 0; index < argv.times; index++) {
        try {
            ...
            const wrapped: any = new Error(`sendTransaction failed at iteration ${index + 1}/${argv.times}: ${e.message}`);
            wrapped.code = e.code;
            wrapped.cause = e;
            throw wrapped;
        }
  ...
  try {
    ...
    try {
      ...
      if (argv.wait) {
        ...
        await pollUntil(
          async () => (await bridger.getBalance()).sub(bridgerBalanceBefore).eq(expectedMintedAmount),
...
// Factory for send-l1/l2/l3 commands (identical except for chain layer and provider URL).
function makeSendCommand(layer: "l1" | "l2" | "l3", urlProp: string) {
  return {
    command: `send-${layer}`,
    describe: `sends funds between ${layer} accounts`,
    builder: sendCommandBuilder,
    handler: async (argv: any) => {
      argv.provider = new ethers.providers.WebSocketProvider(argv[urlProp]);
      try {
        await runStress(argv, sendTransaction);
      } finally {
        argv.provider.destroy();
...
export const sendL1Command = makeSendCommand("l1", "l1url");
export const sendL2Command = makeSendCommand("l2", "l2url");
export const sendL3Command = makeSendCommand("l3", "l3url");
...
export const sendRPCCommand = {
    ...
    handler: async (argv: any) => {
        ...
        try {
            ...
            const wrapped = new Error(`RPC call ${argv.method} to ${argv.url} failed: ${e.message}`);
...
export const setValidKeysetCommand = {
    ...
    handler: async (argv: any) => {
        ...
        try {
            ...
            let keyset: string;
            try {
                keyset = fs.readFileSync(path.join(consts.configpath, "l2_das_keyset.hex")).toString();
            } catch (e: any) {
                throw new Error(`Failed to read l2_das_keyset.hex (has the AnyTrust keyset been generated with --init --l2-anytrust?): ${e.message}`);
            }
